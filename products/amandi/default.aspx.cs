...
namespace ayzhuangxiu.products.amandi
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void LoadProdClass()
        {
            ...
            result.Clear();
            string tsql = "select t.* from ay_prodclass t where bParent=90 or bParent in (select bId from ay_prodclass where bParent=90) order by t.bAddTime";
            DataSet ds = PaducnSoft.DBUtility.DbHelperOleDb.Query(tsql);

            DataRow[] drsRoot = ds.Tables[0].Select("bParent=90", "bOrder");
        ...
        private void BindDataList()
        {
            ...
            string sql = "select a.* from ay_products_v a where (a.bClassID in (select bId from ay_prodclass where bParent=90) or a.bParentID in (select bId from ay_prodclass where bParent=90)) ";
            if (classid != "")
            {
