...
namespace ayzhuangxiu.products.jycom
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void LoadProdClass()
        {
            ...
            result.Clear();
            string tsql = "select t.* from ay_prodclass t where bParent=70 or bParent in (select bId from ay_prodclass where bParent=70) order by t.bAddTime";
            DataSet ds = PaducnSoft.DBUtility.DbHelperOleDb.Query(tsql);

            DataRow[] drsRoot = ds.Tables[0].Select("bParent=70", "bOrder");
        ...
        private void BindDataList()
        {
            ...
            string sql = "select a.* from ay_products_v a where (a.bClassID in (select bId from ay_prodclass where bParent=70) or a.bParentID in (select bId from ay_prodclass where bParent=70)) ";
            if (classid != "")
            {
