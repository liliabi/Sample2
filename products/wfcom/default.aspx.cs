...
namespace ayzhuangxiu.products.wfcom
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void LoadProdClass()
        {
            ...
            result.Clear();
            string tsql = "select t.* from ay_prodclass t where bParent=49 or bParent in (select bId from ay_prodclass where bParent=49) order by t.bAddTime";
            DataSet ds = PaducnSoft.DBUtility.DbHelperOleDb.Query(tsql);

            DataRow[] drsRoot = ds.Tables[0].Select("bParent=49", "bOrder");
        ...
        private void BindDataList()
        {
            ...
            string sql = "select a.* from ay_products_v a where (a.bClassID in (select bId from ay_prodclass where bParent=49) or a.bParentID in (select bId from ay_prodclass where bParent=49)) ";
            if (classid != "")
            {
